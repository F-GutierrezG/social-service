...
ssh -o StrictHostKeyChecking=no ubuntu@${STAGE_SERVER} "docker run -d --restart always -e 'FLASK_ENV=development' -e 'FLASK_APP=manage.py' -e 'APP_SETTINGS=project.config.DevelopmentConfig' -e 'DATABASE_URL=postgres://postgres:postgres@social-db:5432/social' -e 'DATABASE_TEST_URL=postgres://postgres:postgres@social-db:5432/social_test' -e 'SECRET_KEY=secret_key' -e USERS_SERVICE_URL=$USERS_SERVICE_URL -e AUTH_SERVICE_URL=$AUTH_SERVICE_URL -e 'FACEBOOK_OAUTH_URL=https://www.facebook.com/v3.2/dialog/oauth' -e 'FACEBOOK_CLIENT_ID=2207909899468654' -e 'FACEBOOK_CLIENT_SECRET=0bd8e7bfd8aa8f0a7824e3aeda19961c' -e 'FACEBOOK_REDIRECT_URI=https://stage.onelike.gusisoft.cl/social/facebook/access_token' -e 'FACEBOOK_ACCESS_TOKEN_URL=https://graph.facebook.com/v3.2/oauth/access_token' -e 'AWS_BUCKET_NAME=onelike-stage' --name social --network social-service-network --ip 172.22.0.2 $REGISTRY_REPO/$SOCIAL:$TAG"
