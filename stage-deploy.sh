...
ssh -o StrictHostKeyChecking=no ubuntu@${STAGE_SERVER} "docker run -d -e 'POSTGRES_USER=postgres' -e 'POSTGRES_PASSWORD=postgres' -p 5434:5432 --name social-db --network social-service-network --ip 172.23.0.4 $REGISTRY_REPO/$SOCIAL_DB:$TAG"
ssh -o StrictHostKeyChecking=no ubuntu@${STAGE_SERVER} "docker run -d -e 'API_URL=definitions/swagger.yml' -p 8082:8080 --name social-swagger --network social-service-network --ip 172.23.0.3 $REGISTRY_REPO/$SWAGGER:$TAG"
