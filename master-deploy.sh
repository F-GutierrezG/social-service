...
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} "docker run --log-driver=awslogs --log-opt awslogs-region=us-east-2 --log-opt awslogs-group=SocialServiceSwagger -d --restart always -e 'API_URL=definitions/swagger.yml' --name social-swagger --network social-service-network --ip 172.22.0.3 $REGISTRY_REPO/$SWAGGER:$TAG"
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} "docker run --log-driver=awslogs --log-opt awslogs-region=us-east-2 --log-opt awslogs-group=SocialService -d --restart always -e 'FLASK_ENV=production' -e 'FLASK_APP=manage.py' -e 'APP_SETTINGS=project.config.ProductionConfig' -e 'DATABASE_URL=postgres://$DATABASE_PROD_USER:$DATABASE_PROD_PASSWORD@$DATABASE_PROD_URL:$DATABASE_PROD_PORT/$DATABASE_PROD_NAME' -e 'SECRET_KEY=$PROD_SECRET_KEY' -e USERS_SERVICE_URL=$PROD_USERS_SERVICE_URL -e AUTH_SERVICE_URL=$PROD_AUTH_SERVICE_URL -e COMPANIES_SERVICE_URL=$PROD_COMPANIES_SERVICE_URL -e NOTIFICATIONS_SERVICE_URL=$PROD_NOTIFICATIONS_SERVICE_URL -e 'AWS_BUCKET_NAME=onelike-prod' -e 'AWS_REGION=us-east-2' --name social --network social-service-network --ip 172.22.0.2 $REGISTRY_REPO/$SOCIAL:$TAG"
